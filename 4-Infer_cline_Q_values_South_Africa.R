...
data.SA <- read.csv("ddRADseq_samples_metadata_South_Africa.csv", sep=";", header=T)
...
# Plot the data
plot(data.SA$dist_SA, data.SA$Q_RFT, pch="+", xlab="distance", ylab="pusillus ancestry", main="Southern Africa contact zone (individuals)", xlim=c(-500, 500))
...
ind.analysis$clines$data <- hzar.doMolecularData1DPops(data.SA$dist_SA, data.SA$Q_RFT, rep(2, nrow(data.SA)))
#ind.analysis$clines$data <- hzar.doNormalData1DPops(data.SA$dist_SA, data.SA$Q_RFT, rep(0, nrow(data.SA)), rep(1, nrow(data.SA)))
...
#ind.analysis$clines$models[["model1"]] <- hzar.makeCline1DNormal(ind.analysis$clines$data,tails="none")
...
#ind.analysis$clines$models[["model2"]] <- hzar.makeCline1DNormal(ind.analysis$clines$data,tails="both")
...
#ind.analysis$clines$models[["model3"]] <- hzar.makeCline1DNormal(ind.analysis$clines$data,tails="left")
...
#ind.analysis$clines$models[["model4"]] <- hzar.makeCline1DNormal(ind.analysis$clines$data,tails="right")

ind.analysis$clines$models <- sapply(ind.analysis$clines$models, hzar.model.addBoxReq,-500, 500,
                                     simplify = F)
...
#ind.analysis$clines$fit$init <- sapply(ind.analysis$clines$models, hzar.first.fitRequest.gC,
#                                       obsData=ind.analysis$clines$data,verbose=F, simplify=F)
...
ind.analysis$clines$fit$init$model1$mcmcParam$chainLength <- 1e5
ind.analysis$clines$fit$init$model1$mcmcParam$burnin <- 1e4
...
ind.analysis$clines$fit$init$model2$mcmcParam$burnin <- 1e5
...
ind.analysis$clines$fit$init$model3$mcmcParam$burnin <- 1e5
...
ind.analysis$clines$fit$init$model4$mcmcParam$burnin <- 1e5
...
deltaL <- round(hzar.get.ML.cline(ind.analysis$clines$results$model.selected)$param.all$deltaL, 1)
deltaR <- round(hzar.get.ML.cline(ind.analysis$clines$results$model.selected)$param.all$deltaR, 1)
tauL <- round(hzar.get.ML.cline(ind.analysis$clines$results$model.selected)$param.all$tauL, 1)
tauR <- round(hzar.get.ML.cline(ind.analysis$clines$results$model.selected)$param.all$tauR, 1)

fz.cline <- hzar.getCredParamRed(ind.analysis$clines$results$model.selected)
width.CI <- center.CI <- deltaL.CI <- deltaR.CI <- tauL.CI <- tauR.CI <- c()
for(i in 1:length(fz.cline$clines)){
  ...
  deltaL.CI <- c(deltaL.CI, (fz.cline$clines[[i]]$param.free$deltaL))
  deltaR.CI <- c(deltaR.CI, (fz.cline$clines[[i]]$param.free$deltaR))
  tauL.CI <- c(tauL.CI, (fz.cline$clines[[i]]$param.free$tauL))
  tauR.CI <- c(tauR.CI, (fz.cline$clines[[i]]$param.free$tauR))}
...
cat("deltaL = ", deltaL, " [ ", round(range(deltaL.CI)[1],1), ";", round(range(deltaL.CI)[2], 1), " ]", sep="")
cat("deltaR = ", deltaR, " [ ", round(range(deltaR.CI)[1],1), ";", round(range(deltaR.CI)[2], 1), " ]", sep="")
cat("tauL = ", tauL, " [ ", round(range(tauL.CI)[1],1), ";", round(range(tauL.CI)[2], 1), " ]", sep="")
cat("tauR = ", tauR, " [ ", round(range(tauR.CI)[1],1), ";", round(range(tauR.CI)[2], 1), " ]", sep="")

# Plot ML cline for best model with 95% CI and print to a file
jpeg("HZAR_cline_best_model.jpg", width=1000, height=1000)
hzar.plot.fzCline(ind.analysis$clines$results$model.selected, pch = "+", main="Cline extoni/pusillus",
                  ylab="pusillus ancestry", xlab="Distance to contact zone", xlim=c(-300,300), cex=2)
...
#Save all the variables to an R environment. Can be loaded with load("HZAR_SA_env.RData")
save.image(file="HZAR_SA_env_0724.RData")
load("HZAR_SA_env.RData")
