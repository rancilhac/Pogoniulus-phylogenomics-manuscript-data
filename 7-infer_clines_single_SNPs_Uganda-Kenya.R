...
#if(l.min+49 < 5378){ l.max <- l.min+49
#} else { l.max <- 5378 }
...
data.U <- read.csv("../ddRADseq_samples_metadata_Uganda-Kenya.csv", sep=",")
#data <- data[,c(1:9, 14:18)]
data.U <- na.omit(data.U)
colnames(data)[1:2] <- c("Sample", "Subspecies")

data.U$dist_UgandaKenya[data.U$Subspecies == "affinis"] <- -data.U$dist_UgandaKenya[data.U$Subspecies == "affinis"]


# read and prepare the data
loci <- fread("Allele_frequencies_Ugande-Kenya_HZAR.csv", header=T)
...
transect <- c()
for(i in pops){
  transect <- c(transect, mean(data.U[data.U$Locality == i,8]))
}

allo.a <- which(pops == "Diani")
allo.c <- which(pops == "Queen_Elizabeth_NP")

diff.freqs <- apply(loci.a, 2, diff.freq, p1=allo.a, p2=allo.c)
